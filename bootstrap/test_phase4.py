...
def test_voxel_terrain():
    ...
    if "BoxGeometry(blockSize - 1" in html or "BoxGeometry(tileSize" in html:
        results.append(TestResult("Voxel blocks", True, "Using BoxGeometry for terrain"))
    else:
...
def test_path_visuals():
    ...
    if "pathColors" in html and "0x3d2817" in html:
        results.append(TestResult("Dark path colors", True, "Path uses dark dirt tones"))
    else:
...
def test_color_palette():
    ...
    if terrain_colors_found >= 1:
        results.append(TestResult("Terrain colors (PRD)", True, f"{terrain_colors_found}/3 PRD colors found"))
    else:
        results.append(TestResult("Terrain colors (PRD)", False, "PRD colors not found"))
...
def test_decor():
    ...
    if "decorGeo" in html and "decorMat" in html:
        results.append(TestResult("Decorative stones", True, "Decorative elements present"))
    else:
...
def test_mesh_standard_material():
    ...
    if has_standard_material and has_roughness:
        results.append(TestResult("PBR materials", True, "MeshStandardMaterial with roughness for depth"))
    else:
        reasons = []
...
def test_tile_edge_lines():
    ...
    if has_edges_geometry and has_line_segments:
        results.append(TestResult("Tile edge lines", True, "EdgesGeometry + LineSegments for tile depth"))
    else:
        reasons = []
...
def test_tile_gap_spacing():
    ...
    if has_tile_gap:
        results.append(TestResult("Tile gap spacing", True, "Visible gaps between tiles for depth"))
    else:
        results.append(TestResult("Tile gap spacing", False, "No tile gap — tiles may look flat"))
...
def test_lighting_setup():
    ...
    if has_shadow_map and has_fog:
        results.append(TestResult("Advanced lighting", True, "Shadow maps + fog for depth"))
    else:
        reasons = []
...
def test_enemy_distinct_features():
    ...
    has_enemy_features = bool(re.search(
        r"(eyeGeo|pupilGeo|visorGeo|hornGeo|drillGeo|armor|speed.*line|pulsing.*core)",
        html
    ))
    
    if has_enemy_features:
        results.append(TestResult("Enemy distinct features", True, "Enemies have unique visual features"))
    else:
        results.append(TestResult("Enemy distinct features", False, "Enemies are generic shapes"))
...
def run_all_tests():
    ...
    test_groups = [
        ...
        ("4.5 REGRESSION: Visual Quality Guards", lambda: [
