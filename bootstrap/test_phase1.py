...
def test_config_values():
    ...
    if re.search(r"BASE_LIVES:\s*15", html):
        ...
        results.append(TestResult("CONFIG.BASE_LIVES=15", False, "Base lives not set to 15"))
    
    if re.search(r"MAX_ENERGY:\s*80", html):
        results.append(TestResult("CONFIG.MAX_ENERGY=80", True, "Max energy is 80"))
    else:
        results.append(TestResult("CONFIG.MAX_ENERGY=80", False, "Max energy not set to 80"))
    
    if re.search(r"ENERGY_REGEN:\s*1", html):
        results.append(TestResult("CONFIG.ENERGY_REGEN=1", True, "Energy regen is 1/sec"))
    else:
        results.append(TestResult("CONFIG.ENERGY_REGEN=1", False, "Energy regen not set to 1"))
    
    if re.search(r"PREPARATION_TIME:\s*45", html):
        results.append(TestResult("CONFIG.PREPARATION_TIME=45", True, "Preparation time is 45s"))
    else:
        results.append(TestResult("CONFIG.PREPARATION_TIME=45", False, "Preparation time not set to 45"))
...
def test_game_state():
    ...
    if re.search(r"gamePhase:\s*['\"]PREPARATION['\"]", html):
        results.append(TestResult("gameState.gamePhase", True, "gamePhase property exists"))
    else:
        results.append(TestResult("gameState.gamePhase", False, "gamePhase property missing"))
    
    if re.search(r"preparationTimer:", html):
        results.append(TestResult("gameState.preparationTimer", True, "preparationTimer property exists"))
    else:
        results.append(TestResult("gameState.preparationTimer", False, "preparationTimer property missing"))
    
    if re.search(r"pathGrid:\s*\[\]", html):
        results.append(TestResult("gameState.pathGrid", True, "pathGrid property exists"))
    else:
        results.append(TestResult("gameState.pathGrid", False, "pathGrid property missing"))
    
    if re.search(r"maxWaveReached:", html):
        results.append(TestResult("gameState.maxWaveReached", True, "maxWaveReached property exists"))
    else:
        results.append(TestResult("gameState.maxWaveReached", False, "maxWaveReached property missing"))
...
def test_path_generator():
    ...
    if re.search(r"class PathGenerator", html):
        results.append(TestResult("PathGenerator class", True, "PathGenerator class exists"))
    else:
        results.append(TestResult("PathGenerator class", False, "PathGenerator class missing"))
    
    if re.search(r"static createPath", html):
        results.append(TestResult("PathGenerator.createPath", True, "createPath method exists"))
    else:
        results.append(TestResult("PathGenerator.createPath", False, "createPath method missing"))
    
    path_types = ['STRAIGHT', 'WINDING', 'BRANCHING', 'MAZE']
    for ptype in path_types:
        if re.search(rf"['\"]?{ptype}['\"]?", html):
            results.append(TestResult(f"Path type: {ptype}", True, f"{ptype} path type found"))
        else:
            results.append(TestResult(f"Path type: {ptype}", False, f"{ptype} path type not found"))
    
    if re.search(r"static validatePath", html):
        results.append(TestResult("PathGenerator.validatePath", True, "validatePath method exists"))
    else:
        results.append(TestResult("PathGenerator.validatePath", False, "validatePath method missing"))
...
def test_wave_scaling():
    ...
    if re.search(r"function calculateWaveDifficulty", html):
        results.append(TestResult("calculateWaveDifficulty", True, "Wave difficulty formula exists"))
    else:
        results.append(TestResult("calculateWaveDifficulty", False, "Wave difficulty formula missing"))
    
    if re.search(r"function calculateEnemyCount", html):
        results.append(TestResult("calculateEnemyCount", True, "Enemy count formula exists"))
    else:
        results.append(TestResult("calculateEnemyCount", False, "Enemy count formula missing"))
    
    if re.search(r"function calculatePathLength", html):
        results.append(TestResult("calculatePathLength", True, "Path length formula exists"))
    else:
        results.append(TestResult("calculatePathLength", False, "Path length formula missing"))
    
    if re.search(r"Math\.pow\(1\.15", html):
        results.append(TestResult("Difficulty scaling (1.15)", True, "15% difficulty increase per wave"))
    else:
        results.append(TestResult("Difficulty scaling (1.15)", False, "15% difficulty increase not found"))
...
def test_enemy_types():
    ...
    for etype, desc in enemy_types.items():
        if re.search(rf"case ['\"]?{etype}['\"]?:", html):
            ...
            results.append(TestResult(f"Enemy type: {etype}", False, f"{desc} not found"))
...
def test_preparation_phase():
    ...
    if re.search(r"function startGame\(\)", html):
        results.append(TestResult("startGame function", True, "startGame function exists"))
    else:
        results.append(TestResult("startGame function", False, "startGame function missing"))
    
    if re.search(r"function startPreparationPhase", html):
        results.append(TestResult("startPreparationPhase", True, "Preparation phase function exists"))
    else:
        results.append(TestResult("startPreparationPhase", False, "Preparation phase function missing"))
    
    if re.search(r"function startWaveActive\(\)", html):
        results.append(TestResult("startWaveActive", True, "Wave active function exists"))
    else:
        results.append(TestResult("startWaveActive", False, "Wave active function missing"))
    
    if re.search(r"gameState\.gamePhase === ['\"]PREPARATION['\"]", html):
        results.append(TestResult("Preparation timer in animate", True, "Timer logic in game loop"))
    else:
        results.append(TestResult("Preparation timer in animate", False, "Timer logic not in game loop"))
...
def test_turret_destruction():
    ...
    if re.search(r"function destroyOverlappingTurrets", html):
        results.append(TestResult("destroyOverlappingTurrets", True, "Turret destruction function exists"))
    else:
        results.append(TestResult("destroyOverlappingTurrets", False, "Turret destruction function missing"))
    
    if re.search(r"\* 0\.8", html):
        results.append(TestResult("Energy refund (80%)", True, "80% energy refund on turret destruction"))
    else:
        results.append(TestResult("Energy refund (80%)", False, "Energy refund calculation missing"))
    
    if re.search(r"pathGrid\[tower\.gridX\]\[tower\.gridY\]", html):
        results.append(TestResult("pathGrid collision check", True, "Collision detection using pathGrid"))
    else:
        results.append(TestResult("pathGrid collision check", False, "pathGrid collision check missing"))
...
def test_energy_regen():
    ...
    if re.search(r"lastEnergyRegen", html):
        results.append(TestResult("Energy regen variable", True, "Energy regen tracking exists"))
    else:
        results.append(TestResult("Energy regen variable", False, "Energy regen tracking missing"))
    
    if re.search(r"energy \+ CONFIG\.ENERGY_REGEN", html):
        results.append(TestResult("Energy +1 per second", True, "Energy adds every second"))
    else:
        results.append(TestResult("Energy +1 per second", False, "Energy regen logic not found"))
    
    if re.search(r"Math\.min\(CONFIG\.MAX_ENERGY", html):
        results.append(TestResult("MAX_ENERGY cap", True, "Energy capped at max"))
    else:
        results.append(TestResult("MAX_ENERGY cap", False, "Energy cap not implemented"))
...
def test_wave_completion():
    ...
    if re.search(r"function waveComplete\(\)", html):
        results.append(TestResult("waveComplete function", True, "Wave completion function exists"))
    else:
        results.append(TestResult("waveComplete function", False, "Wave completion function missing"))
    
    if re.search(r"energy \+ 20", html):
        results.append(TestResult("Wave completion bonus", True, "+20 energy on wave complete"))
    else:
        results.append(TestResult("Wave completion bonus", False, "Wave bonus not found"))
    
    if re.search(r"startPreparationPhase\(\)", html):
        results.append(TestResult("Transition to preparation", True, "Next wave enters preparation"))
    else:
        results.append(TestResult("Transition to preparation", False, "Transition logic missing"))
...
def test_ui_updates():
    ...
    if re.search(r"wave-timer-val", html):
        results.append(TestResult("Wave timer UI element", True, "Timer display in HTML"))
    else:
        results.append(TestResult("Wave timer UI element", False, "Timer display missing"))
    
    if re.search(r"preparationTimer", html):
        results.append(TestResult("Preparation timer display", True, "Timer shown in UI"))
    else:
        results.append(TestResult("Preparation timer display", False, "Timer not in UI"))
    
    if re.search(r"WAVE \$\{gameState\.wave\}", html):
        results.append(TestResult("Dynamic wave button", True, "Button shows wave number"))
    else:
        results.append(TestResult("Dynamic wave button", False, "Button doesn't show wave"))
...
def test_no_duplicate_functions():
    ...
    if not duplicates:
        results.append(TestResult("No duplicate functions", True, "All functions defined once"))
    else:
        dup_list = ", ".join(f"{name}({count}x)" for name, count in duplicates.items())
        results.append(TestResult("No duplicate functions", False, f"Duplicates found: {dup_list}"))
...
def test_no_undefined_config_refs():
    ...
    safe_refs = {'MAP_WIDTH', 'MAP_HEIGHT', 'TILE_SIZE', 'PATH_COLOR', 'TERRAIN_COLOR',
                 'BASE_ENERGY', 'BASE_LIVES', 'ENERGY_REGEN', 'MAX_ENERGY', 'PREPARATION_TIME'}
    undefined = all_refs - defined_keys - safe_refs
    
    if not undefined:
        results.append(TestResult("No undefined CONFIG refs", True, "All CONFIG refs are defined"))
    else:
        ref_list = ", ".join(f"CONFIG.{k}" for k in sorted(undefined))
        results.append(TestResult("No undefined CONFIG refs", False, f"Undefined: {ref_list}"))
...
def test_no_double_script_tags():
    ...
    if "</script></script>" in html:
        results.append(TestResult("No double script tags", False, "Found </script></script>"))
    else:
        results.append(TestResult("No double script tags", True, "Script tags are clean"))
...
def test_no_extra_css_braces():
    ...
    if style_match:
        ...
        if re.search(r"\}\s*\n\s+\}", css_no_kf):
            results.append(TestResult("No extra CSS braces", False, "Found double closing braces in CSS"))
        else:
            results.append(TestResult("No extra CSS braces", True, "CSS braces are balanced"))
    else:
        results.append(TestResult("No extra CSS braces", False, "Could not find <style> block"))
...
def test_enemy_death_cleanup():
    ...
    has_scene_remove_on_death = bool(re.search(
        r"(health\s*<=\s*0|health\s*<\s*1).*?scene\.remove",
        html, re.DOTALL
    ))
    # Also check for the pattern: createExplosion + scene.remove together (proper cleanup)
    has_explosion_cleanup = bool(re.search(
        r"createExplosion.*?scene\.remove.*?enemy",
        html, re.DOTALL
    ))
    # Make sure there's NO silent filter without cleanup
    has_silent_filter = bool(re.search(
        r"enemies\s*=\s*enemies\.filter.*?health",
        html
    ))
    
    if (has_scene_remove_on_death or has_explosion_cleanup) and not has_silent_filter:
        results.append(TestResult("Enemy death cleanup", True, "Dead enemies removed from scene"))
    else:
        reasons = []
...
def test_projectile_has_target():
    ...
    has_target_assignment = bool(re.search(
    ...
    has_null_target = bool(re.search(
        r"target:\s*null",
        html
    ))
    
    if has_target_assignment and not has_null_target:
        results.append(TestResult("Projectile targeting", True, "Projectiles lock onto enemies"))
    else:
        reasons = []
        if has_null_target:
...
def run_all_tests():
    ...
    test_groups = [
        ...
        ("1.11 REGRESSION: Code Quality Guards", lambda: [
