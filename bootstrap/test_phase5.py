...
def test_leaderboard():
    ...
    if "showGameStats" in html:
        results.append(
            TestResult("Leaderboard display", True, "Stats shown on game over")
        )
    else:
...
def test_stats_tracking():
    ...
    if "pathChanges" in html:
        results.append(
            TestResult("pathChanges tracking", True, "Path change counter exists")
        )
    else:
    ...
    if "bossesDefeated" in html:
        results.append(
            TestResult("bossesDefeated tracking", True, "Boss kill counter exists")
        )
    else:
...
def test_energy_rewards():
    ...
    for pattern, expected in rewards.items():
        if pattern in html:
            results.append(
                TestResult(f"Energy reward: {pattern}", True, f"Reward = {expected}")
            )
