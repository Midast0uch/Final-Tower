...
def test_basic_enemies():
    ...
    if "case 'sniper':" in html and "OctahedronGeometry(9" in html and "#845ef7" in html:
...
def test_new_enemies():
    ...
    if "case 'burrowing':" in html and "CylinderGeometry(8" in html and "#a0522d" in html:
        results.append(TestResult("Burrowing Mole", True, "Brown cylinder, immune to ground"))
    else:
        results.append(TestResult("Burrowing Mole", False, "Missing or incorrect"))
    
    # Spawner mole - pink icosahedron
    if "case 'spawner':" in html and "IcosahedronGeometry(12" in html and "#e599f7" in html:
        results.append(TestResult("Spawner Mole", True, "Pink icosahedron, spawns minions"))
    else:
        results.append(TestResult("Spawner Mole", False, "Missing or incorrect"))
    
    # Boss mole - red, large
    if "case 'boss':" in html and "IcosahedronGeometry(20" in html and "#ff6b6b" in html:
        results.append(TestResult("Boss Mole", True, "Red large icosahedron, every 5 waves"))
    else:
...
def test_enemy_spawning():
    ...
    if "function generateWaveEnemies" in html:
        results.append(TestResult("generateWaveEnemies function", True, "Wave enemy generator exists"))
    else:
    ...
    if "waveNum % 5 === 0" in html and "type = 'boss'" in html:
        results.append(TestResult("Boss every 5 waves", True, "Boss spawns on wave 5, 10, 15..."))
    else:
        results.append(TestResult("Boss every 5 waves", False, "Boss timing not correct"))
...
def test_enemy_scaling():
    ...
    if "waveNum *" in html:
        results.append(TestResult("Health scales with wave", True, "HP increases per wave"))
    else:
        results.append(TestResult("Health scales with wave", False, "No HP scaling"))
    
    # Speed scales with wave
    if "2 + (waveNum * 0.2)" in html:
        results.append(TestResult("Speed scales with wave", True, "Speed increases per wave"))
    else:
...
def test_enemy_death_proper_cleanup():
    ...
    has_silent_filter = bool(re.search(
        r"enemies\s*=\s*enemies\.filter\s*\(\s*e\s*=>.*health",
        html
    ))
    ...
    if has_geometry_dispose:
        results.append(TestResult("Geometry disposal on death", True, "Geometries disposed to prevent leaks"))
    else:
        results.append(TestResult("Geometry disposal on death", False, "No geometry.dispose() found"))
...
def test_no_orphaned_glowmesh_refs():
    ...
    glowmesh_in_cleanup = bool(re.search(
        r"enemy\.glowMesh.*scene\.remove",
        html
    ))
    
    if not glowmesh_in_cleanup:
        results.append(TestResult("No orphaned glowMesh refs", True, "No orphaned glowMesh references"))
    else:
        results.append(TestResult("No orphaned glowMesh refs", False, "glowMesh referenced in cleanup — may cause errors"))
...
def test_slow_debuff_applied():
    ...
    if has_slow_flag and has_slow_factor and has_slow_in_movement:
        results.append(TestResult("Slow debuff applied", True, "SLOW tower applies speed debuff"))
    else:
        reasons = []
...
def test_trap_proximity_trigger():
    ...
    has_proximity_check = bool(re.search(r"TRAP.*distance|TRAP.*dist.*range|tower\.triggered", html, re.DOTALL))
    has_trap_explosion = bool(re.search(r"TRAP.*createExplosion|trap.*explosion", html, re.DOTALL))
    
    if has_proximity_check:
        results.append(TestResult("Trap proximity trigger", True, "TRAP tower triggers on proximity"))
    else:
        results.append(TestResult("Trap proximity trigger", False, "No proximity trigger for TRAP tower"))
...
def run_all_tests():
    ...
    test_groups = [
        ...
        ("2.5 REGRESSION: Enemy Lifecycle Guards", lambda: [
