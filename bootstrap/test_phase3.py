...
def test_turret_types():
    ...
    if (
        "SNIPER:" in html
        and "cost: 40" in html
        and "damage: 15" in html
        and "range: 18" in html
    ):
        results.append(
            TestResult("Sniper Turret", True, "cost=40, damage=15, range=18")
        )
    else:
    ...
    if (
        "TRAP:" in html
        and "cost: 35" in html
        and "oneTime: true" in html
        and "damage: 30" in html
    ):
        results.append(
            TestResult("Trap Turret", True, "cost=35, one-time use, high damage")
        )
    else:
...
def test_turret_fire_rates():
    ...
    if "fireRate:" in html:
        results.append(
            TestResult("Fire rates defined", True, "All turrets have fireRate")
        )
    else:
...
def test_turret_visuals():
    ...
    if "function createTower" in html:
        results.append(
            TestResult("createTower function", True, "Turret creation function exists")
        )
    else:
...
def test_all_towers_have_mesh():
    ...
    tower_func = re.search(
        r"function createTower\([^)]*\)\s*\{(.*?)\n        \}", html, re.DOTALL
    )
    if not tower_func:
        results.append(
            TestResult(
                "All towers have mesh", False, "Could not find createTower function"
            )
        )
        return results
    ...
    if not missing:
        results.append(
            TestResult(
                "All towers have mesh", True, "All 5 tower types have visual geometry"
            )
        )
    else:
        results.append(
            TestResult(
                "All towers have mesh",
                False,
                f"Missing visuals for: {', '.join(missing)}",
            )
        )
...
def test_projectile_creation_has_target():
    ...
    if has_enemy_lookup and not has_null_target:
        results.append(
            TestResult(
                "Projectile target assignment",
                True,
                "Projectiles lock onto enemies at creation",
            )
        )
    else:
        reasons = []
        ...
        results.append(
            TestResult("Projectile target assignment", False, "; ".join(reasons))
        )
...
def test_cannon_area_damage():
    ...
    has_area_loop = bool(
        re.search(r"areaRange|area.*damage|forEach.*health.*-=.*damage", html)
    )

    if has_area_loop:
        results.append(
            TestResult(
                "Cannon area damage", True, "CANNON damages all enemies in radius"
            )
        )
    else:
        results.append(
            TestResult("Cannon area damage", False, "No area damage logic found")
        )
...
def test_no_duplicate_remove_enemy():
    ...
    if count == 1:
        results.append(
            TestResult("Single removeEnemy", True, "Only one removeEnemy definition")
        )
    elif count == 0:
        results.append(
            TestResult("Single removeEnemy", False, "No removeEnemy function found")
        )
    else:
        results.append(
            TestResult(
                "Single removeEnemy",
                False,
                f"Found {count} removeEnemy definitions (duplicates)",
            )
        )
...
def run_all_tests():
    ...
    test_groups = [
        ...
        (
            "3.4 REGRESSION: Tower Combat Guards",
            lambda: [
